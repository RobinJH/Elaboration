    ...
    with open(f) as file:
        for line in file:
            ...
            if match:
                ...
                child_root = ""
                if child_match:
                    print("+++ Possible child package included in {} ({})".format(f.name, with_match))
...
def parse_ada_file(f, with_data):
    """
    ...
    Input: f => Path object pointing to the file to parse
           with_data => Dictionary object to populate
    Returns: None

    """
    assert isinstance(with_data, dict), "*** parse_ada_file: ERROR Not passed a dictionary as 'with_data'"
...
def parse_dir(cwd, with_data):
    ...
    assert isinstance(with_data, dict), "*** parse_dir: ERROR Not passed a dictionary as 'with_data'"
...
def build_with_dictionary(cwd):
    ...
    with_data = {}      # Dictionary to populate

    # Build the dictionary of packages and what they include
    parse_dir(cwd, with_data)
...
def parse_withs(start, with_data, with_stack, circular_stacks):
    """
    ...
    assert isinstance(with_data, dict), "*** parse_withs: ERROR Not passed a dictionary as 'with_data'"
    assert isinstance(with_stack, deque), "*** parse_withs: ERROR Not passed a deque as 'with_stack'"
    assert isinstance(circular_stacks, list), "*** parse_withs: ERROR Not passed a dictionary as 'circular_stacks'"

    if "usr_conf_contribution" in with_stack:
        print("*** Current Stack {}".format(with_stack))
        ...
        for withs in with_data[start]:
            if withs in with_stack:
                ...
                if withs in with_data:
                    ...
                    print("--- Checking stack -> {}".format(with_stack))
                    parse_withs(withs, with_data, with_stack, circular_stacks)
                    with_stack.pop()
...
if __name__ == "__main__":
    ...
    with_data = build_with_dictionary(cwd)
    ...
    for key in with_data.keys():
        ...
        with_stack.clear()
        with_stack.append(key)
        parse_withs(key, with_data, with_stack, circular_stacks)
